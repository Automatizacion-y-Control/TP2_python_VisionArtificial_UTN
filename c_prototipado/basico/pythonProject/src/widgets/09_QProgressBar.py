...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QPushButton, QLineEdit, QCheckBox, QRadioButton, QComboBox, QSlider, QProgressBar
...
# Agrega un QComboBox (selector desplegable) dentro de la ventana
combo = QComboBox(widget)
combo.addItems(['Opción 1', 'Opción 2', 'Opción 3'])
combo.move(400, 120)

# Agrega un QSlider (control deslizante)
slider = QSlider(Qt.Horizontal, widget)
slider.setGeometry(50, 200, 250, 30)
slider.setMinimum(0)
slider.setMaximum(100)
slider.setValue(50)

# Agrega una QProgressBar (barra de progreso)
progress_bar = QProgressBar(widget)
progress_bar.setGeometry(50, 300, 250, 30)
...
# Conecta la selección del combo a una función que imprime la opción seleccionada
combo.currentTextChanged.connect(lambda text: print(f"Seleccionaste: {text}"))

# Conecta el cambio de valor del slider
slider.valueChanged.connect(lambda: print(f"Valor del slider: {slider.value()}"))

# Conecta el cambio de valor del slider para actualizar la barra de progreso
slider.valueChanged.connect(lambda: progress_bar.setValue(slider.value()))
